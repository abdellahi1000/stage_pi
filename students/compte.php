                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Université / École</label>
                                        <p id="display-universite" class="text-lg font-bold text-gray-800">Non précisée</p>
                                    </div>
                                    <div>
    ...
    <div id="settingsSidebar" class="fixed inset-y-0 right-0 w-full md:w-[600px] bg-white shadow-2xl transform translate-x-full transition-transform duration-300 z-50 flex flex-col">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
            <h2 class="text-xl font-black text-gray-900 flex items-center gap-2">
                <i class="fas fa-user-cog text-blue-600"></i> Paramètres du Profil
            </h2>
        ...
        <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
            ...
            <div class="w-full md:w-56 bg-gray-50/50 border-b md:border-b-0 md:border-r border-gray-100 flex md:flex-col overflow-x-auto md:overflow-x-visible no-scrollbar">
                ...
                <button class="settings-tab text-left px-5 py-4 text-xs font-black uppercase tracking-widest text-gray-500 hover:bg-white hover:text-blue-600 border-b-2 md:border-b-0 md:border-l-4 border-transparent flex items-center justify-center md:justify-start gap-4 whitespace-nowrap min-w-max transition-all" data-tab="tab-academic">
                    <i class="fas fa-sliders-h text-lg w-5 text-center"></i> <span>Préférences</span>
                </button>
            </div>
                        ...
                        <div class="flex items-center gap-6 mb-6 p-4 bg-blue-50 rounded-2xl border border-blue-100">
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <button type="button" id="deletePhotoBtn" class="inline-block px-4 py-2 bg-red-100 text-red-600 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-red-200 transition border border-red-200 <?php echo empty($_SESSION['photo_profil']) ? 'hidden' : ''; ?>">
                                        Supprimer la photo
                                    </button>
                                </div>
                    ...
                    <div id="tab-security" class="settings-pane hidden">
                        ...
                        <div class="border-t border-gray-100 my-4 pt-4">
                            ...
                            <div class="space-y-3">
                                ...
                                <input type="password" name="confirm_password" placeholder="Confirmer le nouveau mot de passe" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 focus:bg-white transition">
                            </div>
                        </div>

                        <div class="mb-4 bg-red-50 border border-red-100 rounded-lg p-3">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <p class="text-xs font-bold text-red-700 uppercase tracking-wide mb-1"><i class="fas fa-unlock-alt mr-1"></i> Mot de passe oublié ?</p>
                                    <p class="text-[11px] text-red-600">Définissez un nouveau mot de passe sans votre mot de passe actuel.</p>
                                </div>
                            </div>
                            <div class="mt-2 space-y-2">
                                <input type="password" id="student_reset_pw" placeholder="Nouveau mot de passe" class="w-full bg-white border border-red-200 rounded-lg px-3 py-2 text-sm focus:border-red-400 focus:bg-white transition">
                                <input type="password" id="student_reset_pw_confirm" placeholder="Confirmer le mot de passe" class="w-full bg-white border border-red-200 rounded-lg px-3 py-2 text-sm focus:border-red-400 focus:bg-white transition">
                                <button type="button" id="studentResetPasswordBtn" class="w-full md:w-auto px-4 py-2 bg-red-600 text-white rounded-lg text-xs font-bold hover:bg-red-700 transition shadow-sm">Réinitialiser le mot de passe</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="tab-academic" class="settings-pane hidden">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <?php
                                    require_once __DIR__ . '/../include/lookups.php';
                                    $sm_study_levels = sm_get_study_levels();
                                ?>
                                <select name="niveau_etudes" id="niveau_etudes" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-blue-500 focus:bg-white transition-all outline-none">
                                    ...
                                    <?php foreach ($sm_study_levels as $lvl): ?>
                                        <option value="<?php echo htmlspecialchars($lvl['code']); ?>">
                                            <?php echo htmlspecialchars($lvl['label']); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Université / École</label>
                                <input type="text" name="universite" id="universite" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-blue-500 focus:bg-white transition-all outline-none" placeholder="Ex: ESP / Université de Nouakchott">
                            </div>
                        </div>

                        <div class="mb-6">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Domaine de formation</label>
                            <?php $sm_training_domains = sm_get_training_domains(); ?>
                            <select name="domaine_formation" id="domaine_formation" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-blue-500 focus:bg-white transition-all outline-none">
                                ...
                                <?php foreach ($sm_training_domains as $dom): ?>
                                    <option value="<?php echo htmlspecialchars($dom['code']); ?>">
                                        <?php echo htmlspecialchars($dom['label']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <?php $sm_availability_statuses = sm_get_availability_statuses(); ?>
                            <select name="statut_disponibilite" id="statut_disponibilite" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-blue-500 focus:bg-white transition-all outline-none">
                                <?php foreach ($sm_availability_statuses as $st): ?>
