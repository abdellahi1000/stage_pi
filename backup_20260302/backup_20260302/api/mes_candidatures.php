...
function ensureAcceptanceColumns($db)
{
    ...
    try {
        ...
        // Ensure lm_specifique column
        $check = $db->prepare("SHOW COLUMNS FROM candidatures LIKE 'lm_specifique'");
        $check->execute();
        if ($check->rowCount() === 0) {
            try {
                $db->exec("ALTER TABLE candidatures ADD COLUMN lm_specifique VARCHAR(255) NULL AFTER cv_specifique");
            } catch (PDOException $e) {}
        }
    } catch (Exception $e) {
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($has_cv_col && $has_lm_col) {
            $insert_query = "INSERT INTO candidatures 
                            (user_id, offre_id, message_motivation, statut, cv_specifique, lm_specifique) 
                            VALUES (:user_id, :offre_id, :message, 'pending', :cv_path, :lm_path)";
            ...
            $insert_stmt->execute([
                ...
                ':cv_path' => $cv_path_to_save,
                ':lm_path' => $lm_path_to_save
            ]);
        } elseif ($has_cv_col) {
            $insert_query = "INSERT INTO candidatures 
                            (user_id, offre_id, message_motivation, statut, cv_specifique) 
                            VALUES (:user_id, :offre_id, :message, 'pending', :cv_path)";
            ...
            $insert_query = "INSERT INTO candidatures 
                            (user_id, offre_id, message_motivation, statut) 
                            VALUES (:user_id, :offre_id, :message, 'pending')";
