...
// Ensure company_signature_path exists for enterprise profile/signature UI
try {
...
try {
    ...
    // Total Offers (all)
    $ql = $db->prepare("SELECT COUNT(*) FROM offres WHERE user_id = :uid");
    $ql->execute([':uid' => $user_id]);
    $stats['total_offers'] = (int)$ql->fetchColumn();

    // Hidden/Archived Offers (Offres masquées)
    $qlh = $db->prepare("SELECT COUNT(*) FROM offres WHERE user_id = :uid AND statut = 'archivee'");
    $qlh->execute([':uid' => $user_id]);
    $stats['hidden_offers'] = (int)$qlh->fetchColumn();

    // Active Offers
    $qla = $db->prepare("SELECT COUNT(*) FROM offres WHERE user_id = :uid AND statut = 'active'");
    ...
    // 2. Activity Graph Data (GitHub style) - Last 12 months
    $q3 = $db->prepare("
