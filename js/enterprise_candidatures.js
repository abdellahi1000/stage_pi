...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadApps() {
        ...
        fetch("../api/candidatures.php?action=enterprise_list")
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    allApps = data.candidatures;
                    renderApps(allApps);
                } else {
        ...
        appsGrid.innerHTML = apps.map(app => `
            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all group flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-6 w-full md:w-auto">
                    ...
                    ${app.statut === 'pending' || app.statut === 'en_attente' ? `
                    <div class="flex gap-2">
                        <button onclick="viewAppDetails(${app.id})" class="w-10 h-10 bg-green-50 text-green-600 rounded-xl flex items-center justify-center hover:bg-green-600 hover:text-white transition-all">
                            <i class="fas fa-check"></i>
                        </button>
                        <button onclick="updateStatut(${app.id}, 'rejected')" class="w-10 h-10 bg-rose-50 text-rose-600 rounded-xl flex items-center justify-center hover:bg-rose-600 hover:text-white transition-all">
    ...
    window.viewAppDetails = (id) => {
        ...
        if (specialiteEl) specialiteEl.textContent = app.specialite || "-";
        if (universityEl) universityEl.textContent = app.universite || "-";
        if (domainesEl) domainesEl.textContent = app.domaine_formation || "-";
        if (niveauEl) niveauEl.textContent = app.niveau_etudes || "-";
        if (skillsEl) skillsEl.textContent = app.skills || "-";
        ...
        // CV Logic
        const cvStr = app.cv_specifique || app.profil_cv_path;
        if (cvBtn) {
            if (cvStr) {
                cvBtn.href = cvStr.startsWith("http") ? cvStr : "../" + cvStr;
                cvBtn.classList.remove("opacity-50", "pointer-events-none");
            } else {
                cvBtn.href = "#";
                cvBtn.classList.add("opacity-50", "pointer-events-none");
            }
        }
        ...
        const isClosed = app.statut !== 'pending' && app.statut !== 'en_attente';
        if (isClosed) {
    ...
    window.updateStatut = (id, statut) => {
        if (!confirm(`Voulez-vous vraiment changer le statut en ${statut === 'accepted' ? 'accepté' : 'refusé'} ?`)) return;
        
        fetch("../api/candidatures.php", {
            method: "PUT",
            body: new URLSearchParams({ candidature_id: id, statut: statut === 'accepted' ? 'accepte' : 'refuse' })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
    ...
    if (btnAcceptAppModal) {
        btnAcceptAppModal.addEventListener("click", () => {
             // For Enterprise, we might want to just show a quick status update or redirected to detailed form
             // but per UI rules, we'll keep it simple or align with legacy "Accept with details"
             if (currentApp) updateStatut(currentApp.id, 'accepted');
        });
    }
    ...
    if (btnRejectAppModal) {
        btnRejectAppModal.addEventListener("click", () => {
            if (currentApp) updateStatut(currentApp.id, 'rejected');
        });
    }
